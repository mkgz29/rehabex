...
-- A failed attempt used to consume the idempotency slot for its payment and
-- status forever: the RPC treated ANY existing payment_events row as a
-- duplicate, and three unique indexes physically prevented a second row. A
-- payment rejected by a misconfiguration could therefore never be retried, even
-- after the cause was fixed (observed on payment 178635154627,
-- error_code TEST_MODE_REQUIRED).
--
-- New invariant: at most one *processed* event per (provider, payment, status).
-- Effects stay idempotent; failed and ignored attempts accumulate as auditable
-- history and no longer block a valid retry. The transaction-scoped advisory
-- lock in the function still serializes concurrent attempts, so two retries can
-- never both apply stock.
--
-- Every unique index on payment_events was reviewed. The three that could be
-- reached by a retry are narrowed to processed rows. provider_event_id is left
-- untouched: this integration always writes NULL there, so it never constrains.
...
DROP INDEX IF EXISTS public.idx_payment_events_provider_request_unique;

CREATE UNIQUE INDEX IF NOT EXISTS "idx_payment_events_processed_payment_status_unique"
  ON public.payment_events (provider, provider_payment_id, provider_status)
  WHERE status = 'processed' AND provider_payment_id IS NOT NULL AND provider_status IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS "idx_payment_events_processed_dedupe_unique"
  ON public.payment_events (provider, dedupe_key)
  WHERE status = 'processed' AND dedupe_key IS NOT NULL;

CREATE UNIQUE INDEX IF NOT EXISTS "idx_payment_events_processed_request_unique"
  ON public.payment_events (provider, request_id)
  WHERE status = 'processed' AND request_id IS NOT NULL;

-- Keeps the audit lookups that the dropped unique indexes also served.
CREATE INDEX IF NOT EXISTS "idx_payment_events_provider_payment_status"
  ON public.payment_events (provider, provider_payment_id, provider_status);

...
import { isValid, parseUpdateProductPayload } from '../../../server/admin/validators.js';
import { applyAdminCors, logEvent, type ApiRequest, type ApiResponse } from '../../../server/commerce/commerce.js';

export const config = { api: { bodyParser: false } };

export default createAdminUpdateProductHandler();
...
export function createAdminUpdateProductHandler(overrides: Partial<RequireAdminDependencies> = {}) {
  ...
  return async function handler(request: ApiRequest, response: ApiResponse) {
    ...
    const { data, error } = await auth.rpc.rpc('admin_update_product', {

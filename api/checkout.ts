...
export default async function handler(request: ApiRequest, response: ApiResponse) {
  ...
  const { error: tokenError } = await supabase.from('orders').update({ status_access_token_hash: hash(statusToken) }).eq('id', orderId).eq('idempotency_key', idempotencyKey);
  if (tokenError) {
  ...
  if (preferenceResult.status === 'ready') {
    logEvent('checkout_preference_created', { orderId });
